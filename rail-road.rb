...
class RailRoad
  ...
  # def seed
  #   st1 = Station.new 'Олимпийская деревня'
  #   stations.push(st1)
  #   st2 = Station.new 'Роза Хутор'
  #   stations.push(st2)
  #   st3 = Station.new 'Хоста'
  #   stations.push(st3)
  ...
  def create_route
    ...
    puts "Новый маршрут с начальной станцией #{start_station.name} и конечной станцией #{finish_station.name} успешно создан"
    menu
  end
  ...
  def choose_stations
    ...
    stations_indexes = gets.encode("UTF-8", invalid: :replace).split(",").map { |cmd_number| cmd_number.to_i - 1 }
  end
  ...
  def choose_route
    ...
    routes.each {
        |route| finish_station_index = route.stations.length - 1
        routes_names.push("\"#{route.stations[0].name}-#{route.stations[finish_station_index].name}\"")
    }

    routes_names.each_with_index { |route_name, index|
